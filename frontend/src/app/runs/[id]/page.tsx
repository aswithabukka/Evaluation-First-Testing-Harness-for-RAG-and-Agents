...
import type { EvaluationResult, RegressionItem, RunStatus, TestCase } from "@/types";
...
function MetricGauge({ label, value, threshold = 0.7 }: { label: string; value: number | null | undefined; threshold?: number }) {
    ...
    <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 text-xs">
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1 mb-3">
          <ScoreRow label="Faithfulness" current={currentResult?.faithfulness ?? null} baseline={baselineResult?.faithfulness ?? null} />
          <ScoreRow label="Relevancy" current={currentResult?.answer_relevancy ?? null} baseline={baselineResult?.answer_relevancy ?? null} />
          <ScoreRow label="Precision" current={currentResult?.context_precision ?? null} baseline={baselineResult?.context_precision ?? null} />
        </div>
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1 mb-3">
          <ScoreRow label="Faithfulness" current={baselineResult?.faithfulness ?? null} baseline={null} />
          <ScoreRow label="Relevancy" current={baselineResult?.answer_relevancy ?? null} baseline={null} />
          <ScoreRow label="Precision" current={baselineResult?.context_precision ?? null} baseline={null} />
        </div>
...
export default function RunDetailPage() {
  ...
  // Fetch test cases so we can show query + ground truth alongside each result
  ...
  // Baseline results for side-by-side diff
  ...
  // Baseline run info for pipeline comparison card
  ...
  // Derive a display version: prefer explicit pipeline_version, fall back to config label
  ...
  // Index by test_case_id for O(1) lookup
      ...
      <Card>
        ...
        <CardBody>
          <div className="space-y-5">
            ...
            {/* Pipeline config (auto-captured from adapter) */}
      ...
      {baselineRun && sm && (
        <Card className="border-blue-200">
          <CardHeader>
            ...
            {baselineRun.git_commit_sha && run.git_commit_sha && baselineRun.git_commit_sha !== run.git_commit_sha && (
              <p className="text-xs text-gray-500 mt-0.5">
                Commit:{" "}
                <code className="font-mono">{baselineRun.git_commit_sha.slice(0, 7)}</code>
                {" → "}
                <code className="font-mono">{run.git_commit_sha.slice(0, 7)}</code>
              </p>
            )}
            {/* Side-by-side changelog notes */}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {((): { label: string; base: number | null; curr: number | null }[] => {
                  const bsm = baselineRun.summary_metrics;
                  return [
                    { label: "Pass Rate",         base: bsm?.pass_rate ?? null,             curr: sm.pass_rate },
                    { label: "Faithfulness",       base: bsm?.avg_faithfulness ?? null,      curr: sm.avg_faithfulness },
                    { label: "Answer Relevancy",   base: bsm?.avg_answer_relevancy ?? null,  curr: sm.avg_answer_relevancy },
                    { label: "Context Precision",  base: bsm?.avg_context_precision ?? null, curr: sm.avg_context_precision },
                    { label: "Context Recall",     base: bsm?.avg_context_recall ?? null,    curr: sm.avg_context_recall },
                  ];
                })().map(({ label, base, curr }) => {
                  const delta = base !== null && curr !== null ? curr - base : null;
                  return (
                    <tr key={label} className="hover:bg-blue-50/40">
        ...
        <Card>
          <CardBody>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-6 divide-x divide-gray-100">
              <MetricGauge label="Pass Rate" value={sm.pass_rate} threshold={thresholds.pass_rate ?? 0.8} />
              <MetricGauge label="Faithfulness" value={sm.avg_faithfulness} threshold={thresholds.faithfulness ?? 0.7} />
              <MetricGauge label="Answer Relevancy" value={sm.avg_answer_relevancy} threshold={thresholds.answer_relevancy ?? 0.7} />
              <MetricGauge label="Context Precision" value={sm.avg_context_precision} threshold={thresholds.context_precision ?? 0.6} />
              <MetricGauge label="Context Recall" value={sm.avg_context_recall} threshold={thresholds.context_recall ?? 0.6} />
            </div>
          </CardBody>
        </Card>
      )}
        ...
        <Card className="border-orange-200">
          <CardHeader>
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-semibold text-orange-700">
                ⚠️ {diff.regressions.length} Regression(s) vs Baseline
              </h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {["Query", "Failure Reason", "Faith Δ", "Rel Δ", ""].map((h) => (
                    <th key={h} className="px-4 py-2.5 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {diff.regressions.map((reg) => {
                  const faithDelta = (reg.current_scores.faithfulness ?? 0) - (reg.baseline_scores.faithfulness ?? 0);
                  const relDelta = (reg.current_scores.answer_relevancy ?? 0) - (reg.baseline_scores.answer_relevancy ?? 0);
                  const isExpanded = expandedReg === reg.test_case_id;
                  return (
                    <>
                      <tr
                        key={reg.test_case_id}
                        className={`border-b border-gray-50 hover:bg-orange-50 ${isExpanded ? "bg-orange-50" : ""}`}
                      >
                        ...
                        <td className={`px-4 py-3 text-xs font-mono ${faithDelta < 0 ? "text-red-600" : "text-green-600"}`}>
                          {faithDelta > 0 ? "+" : ""}{faithDelta.toFixed(3)}
                        </td>
                        <td className={`px-4 py-3 text-xs font-mono ${relDelta < 0 ? "text-red-600" : "text-green-600"}`}>
                          {relDelta > 0 ? "+" : ""}{relDelta.toFixed(3)}
                        </td>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {diff.improvements.map((imp) => {
                    const faithDelta = (imp.current_scores.faithfulness ?? 0) - (imp.baseline_scores.faithfulness ?? 0);
                    const relDelta = (imp.current_scores.answer_relevancy ?? 0) - (imp.baseline_scores.answer_relevancy ?? 0);
                    return (
        ...
        {resultsLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {["Result", "Question", "Faithfulness", "Relevancy", "Precision", "Recall", "Rules", "Duration", ""].map((h) => (
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="px-3 py-3 font-mono text-xs">{formatScore(r.faithfulness)}</td>
                        <td className="px-3 py-3 font-mono text-xs">{formatScore(r.answer_relevancy)}</td>
                        <td className="px-3 py-3 font-mono text-xs">{formatScore(r.context_precision)}</td>
                        <td className="px-3 py-3 font-mono text-xs">{formatScore(r.context_recall)}</td>
                        <td className="px-3 py-3">
